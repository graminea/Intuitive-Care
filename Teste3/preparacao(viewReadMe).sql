...
RETURNS void AS $$
    ...
    os.makedirs("Teste3/tabelas_csv", exist_ok=True)  
    for i in range(3, 5):  # Loop de anos 2023 e 2024
        for k in range(1, 5):  # Trimestres 1 a 4
            ...
            with open(f"Teste3
    /tabelas_csv/{k}T202{i}.zip", "wb") as file:
                file.write(response.content)
            
            with zipfile.ZipFile(f"Teste3
    /tabelas_csv/{k}T202{i}.zip", "r") as zip_file:
                zip_file.extractall("Teste3
        /tabelas_csv/")  # Extrai todos os arquivos do zip para o diretório especificado
            

            os.remove(f"Teste3
    /tabelas_csv/{k}T202{i}.zip")

    requests.get("https://dadosabertos.ans.gov.br/FTP/PDA/operadoras_de_plano_de_saude_ativas/Relatorio_cadop.csv",
                                "Teste3
                        /tabelas_csv/Relatorio_cadop.csv")
